...
const fromDb = o => ({
  ...
  city: o.city||"", language: o.language||"Arabic", adSource: o.ad_source||"Facebook",
...
function Toast({ msg, type }) {
  if (!msg) return null;
  return <div style={{ position:"fixed", bottom:24, right:24, background:type==="error"?"#ef4444":"#10b981", color:"#fff", borderRadius:10, padding:"12px 20px", fontWeight:700, fontSize:14, zIndex:9999, boxShadow:"0 8px 24px rgba(0,0,0,.2)" }}>{msg}</div>;
}
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f172a 0%,#1e293b 50%,#0f172a 100%)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <div style={{ background:"#fff", borderRadius:24, padding:"48px 40px", width:"100%", maxWidth:420, textAlign:"center", boxShadow:"0 32px 96px rgba(0,0,0,.4)" }}>
        ...
        <button onClick={handleLogin} disabled={loading} style={{
          ...
          gap:12, boxShadow:"0 2px 8px rgba(0,0,0,.08)", opacity:loading?0.7:1,
        }}>
          {loading ? (
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const query = isAdmin
      ...
      : supabase.from("leads").select("*").eq("agent", agentName).order("created_at",{ascending:false});
    query.then(({data,error})=>{
      if(error) showToast("Failed to load leads","error");
      else setLeads((data||[]).map(fromDb));
      setLoading(false);
    });
  },[user]);
  ...
  const filtered=useMemo(()=>leads.filter(l=>{
    ...
    return (!q||l.leadName.toLowerCase().includes(q)||l.phone.includes(q)||l.adCampaign?.toLowerCase().includes(q))
      &&(fDisp==="All"||l.disposition===fDisp)&&(fSrc==="All"||l.adSource===fSrc)&&(fProd==="All"||l.product===fProd);
  }).sort((a,b)=>sortBy==="leadName"?a.leadName.localeCompare(b.leadName):sortBy==="attempts"?b.attemptCount-a.attemptCount:(b.dateReceived+b.timeReceived).localeCompare(a.dateReceived+a.timeReceived)),[leads,search,fDisp,fSrc,fProd,sortBy]);

  const add=async form=>{
    setSaving(true);
    const {data,error}=await supabase.from("leads").insert(toDb(form)).select().single();
    if(error) showToast("Failed to add lead","error");
    else { setLeads(ls=>[fromDb(data),...ls]); setShowAdd(false); showToast("✅ Lead added!"); }
    setSaving(false);
  };

  const upd=async(id,patch)=>{
    setSaving(true);
    const {data,error}=await supabase.from("leads").update(toDb(patch)).eq("id",id).select().single();
    if(error) showToast("Failed to update","error");
    else { const u=fromDb(data); setLeads(ls=>ls.map(l=>l.id===id?u:l)); if(detail?.id===id) setDetail(u); setDispLead(null); setEditLead(null); showToast("✅ Updated!"); }
    setSaving(false);
  };
  ...
  const del=async id=>{
    ...
    else { setLeads(ls=>ls.filter(l=>l.id!==id)); setDelLead(null); setDetail(null); showToast("🗑️ Deleted"); }
    setSaving(false);
  };
  ...
  const fb=leads.filter(l=>l.adSource==="Facebook").length,ig=leads.filter(l=>l.adSource==="Instagram").length,wa=leads.filter(l=>l.adSource==="WhatsApp Click-to-Chat").length;
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Segoe UI',sans-serif", background:"#f0f2f8", minHeight:"100vh" }}>
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #e9ecf3", padding:"0 22px" }}>
        <div style={{ maxWidth:1500, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:56 }}>
          ...
          <div style={{ display:"flex", gap:3, background:"#f0f2f8", borderRadius:10, padding:3 }}>
            {navB("leads","📋 Leads")}{navB("callbacks","🔁 Callbacks",urgentCbs)}{navB("analytics","📊 Analytics")}
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
      ...
      <div style={{ maxWidth:1500, margin:"0 auto", padding:"20px 22px" }}>
        ...
        {isAdmin&&<div style={{ background:"#fef3c7", border:"1px solid #fde68a", borderRadius:10, padding:"10px 16px", marginBottom:16, fontSize:13 }}>👑 <span style={{ fontWeight:600, color:"#92400e" }}>Admin view — ALL leads from all agents</span></div>}
                ...
                <tbody>
                  {filtered.map(l=>{
                    ...
                    return (<tr key={l.id} style={{ borderBottom:"1px solid #f1f5f9", background:ov?"#fff5f5":dt?"#fffbeb":"#fff" }}>
                      ...
                      <td style={{ padding:"11px 12px", whiteSpace:"nowrap" }}><div style={{ fontSize:12, fontWeight:700, color:"#0f172a" }}>{l.dateReceived}</div><div style={{ fontSize:11, color:"#94a3b8" }}>{l.timeReceived||"—"}</div></td>
                      <td style={{ padding:"11px 12px" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <Ava name={l.leadName} size={30}/>
                          <div><div style={{ fontWeight:700, color:"#0f172a", cursor:"pointer", textDecoration:"underline dotted" }} onClick={()=>setDetail(l)}>{l.leadName}</div><div style={{ fontSize:11, color:"#94a3b8" }}>{l.city} · {l.gender} · {l.language}</div></div>
                        </div>
                      </td>
                      <td style={{ padding:"11px 12px", whiteSpace:"nowrap" }}>
                        <div style={{ fontWeight:600 }}>{l.phone}</div>
                        <a href={`https://wa.me/${(l.whatsappNumber||"").replace(/\D/g,"")}`} target="_blank" rel="noreferrer" style={{ display:"inline-flex", alignItems:"center", gap:3, marginTop:3, background:"#25d366", color:"#fff", borderRadius:5, padding:"2px 7px", fontSize:10, fontWeight:800, textDecoration:"none" }}>W Chat</a>
                      </td>
                      <td style={{ padding:"11px 12px" }}><SrcBadge source={l.adSource}/></td>
                      <td style={{ padding:"11px 12px" }}><div style={{ fontSize:12, fontWeight:600, color:"#0f172a" }}>{l.adCampaign||"—"}</div><div style={{ fontSize:11, color:"#94a3b8" }}>{l.adSet||""}</div></td>
                      ...
                      <td style={{ padding:"11px 12px" }}><div style={{ display:"flex", alignItems:"center", gap:6 }}><Ava name={l.agent||"?"} size={22}/><span style={{ fontSize:12, color:"#475569" }}>{l.agent}</span></div></td>
                      <td style={{ padding:"11px 12px", textAlign:"center" }}><span style={{ fontWeight:800, fontSize:14, color:l.attemptCount>=4?"#ef4444":l.attemptCount>=2?"#f59e0b":"#0f172a" }}>{l.attemptCount}</span></td>
                      <td style={{ padding:"11px 12px", whiteSpace:"nowrap" }}>
                        {l.callbackDate?(<div style={{ fontSize:11 }}><div style={{ fontWeight:700, color:ov?"#ef4444":dt?"#f59e0b":"#3b82f6" }}>{ov?"🚨 Overdue":dt?"⏰ Today":"📅 "+l.callbackDate}</div><div style={{ color:"#94a3b8" }}>{l.callbackTime}</div></div>):<span style={{ color:"#cbd5e1", fontSize:11 }}>—</span>}
                      </td>
                      <td style={{ padding:"11px 12px" }}>
        ...
        {!loading&&tab==="analytics"&&(
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            ...
            <div style={{ background:"#fff", borderRadius:14, border:"1px solid #e9ecf3", padding:22 }}>
              ...
              {[{src:"Facebook",n:fb,c:"#1877f2"},{src:"Instagram",n:ig,c:"#c13584"},{src:"WhatsApp Click-to-Chat",n:wa,c:"#25d366"}].map(({src,n,c})=>{ const p=total?(n/total)*100:0; return (<div key={src} style={{ marginBottom:18 }}><div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}><span style={{ fontSize:13, fontWeight:700 }}>{src==="WhatsApp Click-to-Chat"?"WhatsApp":src}</span><span style={{ fontSize:13, fontWeight:800, color:c }}>{n} ({p.toFixed(0)}%)</span></div><div style={{ background:"#f1f5f9", borderRadius:8, height:10, overflow:"hidden" }}><div style={{ height:"100%", width:`${p}%`, background:c, borderRadius:8 }}/></div></div>); })}
            </div>
            {isAdmin&&(
              <div style={{ background:"#fff", borderRadius:14, border:"1px solid #e9ecf3", padding:22, gridColumn:"1/-1" }}>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                  <thead><tr style={{ borderBottom:"1px solid #f1f5f9" }}>{["Agent","Leads","Converted","Interested","Rate"].map(h=><th key={h} style={{ padding:"5px 10px", textAlign:"left", fontSize:10, fontWeight:800, color:"#94a3b8" }}>{h}</th>)}</tr></thead>
                  <tbody>
                    {[...new Set(leads.map(l=>l.agent))].filter(Boolean).map(a=>{ const al=leads.filter(l=>l.agent===a),ac=al.filter(l=>l.disposition==="Converted").length,ai=al.filter(l=>l.disposition==="Interested").length,r=((ac/al.length)*100).toFixed(0); return (<tr key={a} style={{ borderBottom:"1px solid #f8f9fc" }}><td style={{ padding:"10px 10px" }}><div style={{ display:"flex", alignItems:"center", gap:8 }}><Ava name={a} size={26}/><span style={{ fontWeight:700 }}>{a}</span></div></td><td style={{ padding:"10px 10px", color:"#475569" }}>{al.length}</td><td style={{ padding:"10px 10px" }}><span style={{ background:"#d1fae5", color:"#059669", borderRadius:20, padding:"1px 8px", fontSize:11, fontWeight:800 }}>{ac}</span></td><td style={{ padding:"10px 10px" }}><span style={{ background:"#ecfdf5", color:"#10b981", borderRadius:20, padding:"1px 8px", fontSize:11, fontWeight:800 }}>{ai}</span></td><td style={{ padding:"10px 10px", fontWeight:900, color:Number(r)>=20?"#059669":"#0f172a" }}>{r}%</td></tr>); })}
                  </tbody>
                </table>
              </div>
            )}
      ...
      <Modal show={!!delLead} onClose={()=>setDelLead(null)} width={380}>
        {delLead&&(<div style={{ padding:32, textAlign:"center" }}>
          <div style={{ fontSize:42, marginBottom:12 }}>🗑️</div>
          <div style={{ fontWeight:900, fontSize:17, marginBottom:8, fontFamily:"'Sora',sans-serif" }}>Delete this lead?</div>
          <div style={{ color:"#64748b", marginBottom:22, fontSize:14 }}>Permanently remove <strong>{delLead.leadName}</strong>?</div>
